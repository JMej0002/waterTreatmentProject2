...
%link the table below to the results!!!
...
$L_0$ & \texttt{[insert]} mg/L & Mixed BOD in receiving stream \\
$t_c$ & \texttt{[insert]} days & Time to reach minimum DO \\
$x_c$ & \texttt{[insert]} m & Distance to minimum DO level \\
$D(t_c)$ & \texttt{[insert]} mg/L & DO deficit at time $t_c$ \\
\hline
\end{tabular}
\end{table}
...
the dissolved oxygen deficit at its critical point is [insert result] mg/L, 
which is [below/above] the regulatory limit of 2.55 mg/L. Therefore, the discharge 
[complies/does not comply] with the receiving waterway's environmental standard.
