...
The detention times required for each are calculated using the following empirical formula:

\begin{align}
t &= \frac{E_r \cdot a}{1 - E_r \cdot b}
\label{eq:time_of_residency}
\end{align}

where \( E_r \) is the removal efficiency, and \( a \), \( b \) are parameters defined for SST and BOD, see Table \ref{tab:detention_time_parameters}. 
...
& \% & Solved from Eq.\eqref{eq:efficiency}, replacing $t_{\text{TSS}}$\\
$V_{\text{clarifier}}^{\text{calc}}$ & 
% BEGIN py_calculatedSedimentationBasinVolume
13589.15
% END py_calculatedSedimentationBasinVolume
& m$^3$ & Calculated with Eq.\eqref{eq:clarifier_volume}\\
$V_{\text{clarifier}}^{\text{max}}$ &
% BEGIN py_maxSedimentationBasinVolume
10513.62
% END py_maxSedimentationBasinVolume
& m$^3$ & Calculated with Eq.\eqref{eq:clarifier_volume}, using max. time residency.\\
$V_{\text{equilization}}^{\text{req}}$ &
% BEGIN py_RequiredEqualizationTankVolume
3075.53
% END py_RequiredEqualizationTankVolume
& m$^3$ & Difference between $V_{\text{clarifier}}^{\text{max}}$ and $V_{\text{clarifier}}^{\text{calc}}$\\
\hline
\end{tabular}
\end{table}

Tha sedimentation basin height is then selected based on the typical design criteria 
(see Appendix~\ref{app:primary_criteria}). The 5 m height upper limit is selected and 0.5 m
of 
