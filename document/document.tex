...
To determine the volume of the primary clarifiers, the removal efficiencies required for suspended solids (SS) 
...
based on the $BOD_5$ content (Eq.\eqref{eq:efficiency}) 
and for the $TSS$ content, the oposite procedure was implemented; based 
on the obtained residence time, the subsequent efficiency is determined. 
...
This volume (Eq.\eqref{eq:clarifier_volume}) is then checked against the acceptable detention time range for 
average flow, 
\(1.5~\text{h} < t < 2.5~\text{h}\), and revised accordingly. 
The excess volume, if any, would be allocated for equalization.
